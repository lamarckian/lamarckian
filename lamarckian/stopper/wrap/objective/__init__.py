    ...
    def decorate(stopper):
        class Stopper(stopper):
            ...
            def __call__(self, outcome, *args, **kwargs):
                ...
                attr.recent += [result['objective'] for result in outcome['results']]
                if len(attr.recent) >= attr.sample:
                    objective = np.mean(attr.recent, axis=0)
                    attr.recent.clear()
                    self.evaluator.recorder.put(lamarckian.util.record.Scalar(self.evaluator.cost, **{f'{tag}/objective{i}': value for i, value in enumerate(objective)}))
                    return super().__call__(outcome, *args, **kwargs, objective=objective)
                return False
        return Stopper
    return decorate
