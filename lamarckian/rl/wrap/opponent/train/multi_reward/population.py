...
def main():
    ...
    styles = state['styles']
    state['population'] = sum([[dict(decision=dict(blob=next(iter(item['blobs'].values())))) for item in itertools.islice(reversed(style.pool), args.limit)] for style in styles.values()], [])
    if args.style is not None:
        style = list(styles.values())[args.style]
        state['decision'] = dict(blob=next(iter(style.pool[-1]['blobs'].values())))
