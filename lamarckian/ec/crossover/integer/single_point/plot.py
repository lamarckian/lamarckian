...
def main():
    ...
    parent1 = np.array([np.random.randint(lower, upper) for lower, upper in zip(*encoding.T)])
    parent2 = np.array([np.random.randint(lower, upper) for lower, upper in zip(*encoding.T)])
    fig = plt.figure()
    ax = fig.gca()
