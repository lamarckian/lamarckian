...
def bitmap(name=None, root=os.path.join('~', 'debug', 'gym'), ext='.bmp'):
    if name is None:
...
def bitmap_stack(name=None, root=os.path.join('~', 'debug', 'gym'), ext='.bmp'):
    if name is None:
        name = inspect.getframeinfo(inspect.currentframe()).function
    root = os.path.join(os.path.expanduser(os.path.expandvars(root)), time.strftime('%Y-%m-%d_%H-%M-%S.%f'), name)
...
def heatmap(name=None, root=os.path.join('~', 'debug', 'gym'), ext='.svg', **kwargs):
    if name is None:
        name = inspect.getframeinfo(inspect.currentframe()).function
    root = os.path.join(os.path.expanduser(os.path.expandvars(root)), time.strftime('%Y-%m-%d_%H-%M-%S.%f'), name)
