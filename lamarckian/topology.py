...
def main():
    ...
    actors = ray.actors()
    for ip, group in toolz.groupby(lambda item: item['Address']['IPAddress'], actors.values()).items():
        print(f"{ip} ({len(group)})")
        for item in sorted(group, key=lambda item: item['Name']):
            print(item['Name'])
        print()
